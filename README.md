...
下载新版安装包直接运行即可**覆盖安装**，无需先卸载：

- 安装目录沿用上次的选择，桌面与开始菜单快捷方式自动更新
- 已导入的文档、解析结果与全部设置自动保留（数据在 `%LOCALAPPDATA%\DocFactory`，与程序目录分离）
- 升级过程不会出现「是否删除数据」的询问——那只在真正卸载时出现
- 新版本若调整了个别设置项，仅该项恢复默认，其余自定义不受影响

检查新版：应用内「设置 → 关于 → 检查更新」，仅在手动触发时访问 GitHub。
...
数据目录为 `%LOCALAPPDATA%\DocFactory`。卸载时会询问是否一并删除解析数据，默认保留，重装后可继续使用。应用不含遥测，不后台联网。
